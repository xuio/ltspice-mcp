...
except Exception:  # pragma: no cover - optional dependency depending on mcp package version
    ...
    async with AsyncExitStack() as stack:
        if args.transport == "stdio":
            ...
            if streamablehttp_client is None:
                raise RuntimeError(
                    "streamable_http client support is unavailable in this environment. "
                    "Upgrade the mcp package or use --transport stdio."
                )
            read_stream, write_stream, _ = await stack.enter_async_context(
                streamablehttp_client(args.server_url)
            )

        session = await stack.enter_async_context(ClientSession(read_stream, write_stream))
        init = await session.initialize()
        print(f"Connected to {init.serverInfo.name} {init.serverInfo.version} via {args.transport}")
        ...
        sim_target = _extract_call_result(
            await session.call_tool(
                "resolveSchematicSimulationTarget",
                {"asc_path": template_schematic.get("asc_path")},
            )
        )
        _require(isinstance(sim_target, dict), "resolveSchematicSimulationTarget did not return an object")
        _require(sim_target.get("can_batch_simulate") is True, f"resolveSchematicSimulationTarget failed: {sim_target}")
        _require(sim_target.get("sidecar_found") is True, "Expected sidecar netlist for template schematic")
        print("Schematic simulation target resolution check passed")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="End-to-end smoke test for ltspice-mcp via MCP stdio or streamable-http transport"
    )
    parser.add_argument(
        "--transport",
        choices=("stdio", "streamable-http"),
        default="stdio",
        help="MCP transport to use for the smoke test session",
    )
    parser.add_argument(
        "--server-url",
        default="http://127.0.0.1:8765/mcp",
        help="HTTP MCP endpoint URL (used when --transport=streamable-http)",
    )
    parser.add_argument(
        "--server-command",
        default="ltspice-mcp",
        help="Command used to launch the MCP server (stdio mode only)",
    )
    parser.add_argument(
        "--server-cwd",
        default=str(Path(__file__).resolve().parent),
        help="Working directory for launching the server (stdio mode only)",
    )
    parser.add_argument(
        "--workdir",
    ...
    parser.add_argument(
        "--ltspice-binary",
        default=None,
        help="Explicit LTspice binary path (stdio mode only)",
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=120,
        help="Simulation timeout passed to the server (stdio mode only)",
    )
